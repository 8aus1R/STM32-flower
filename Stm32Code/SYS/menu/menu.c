...
void Inform_Show(void)
{
    if (!InterFace) { //自动模式页面1
        ...
        oled_ShowNum(48, 4, 100-SensorData.SoliVal, 2, 16);
        oled_ShowNum(48, 6, 100-SensorData.LightVal, 2, 16); 
        oled_ShowString(80, 2, "%", 16);
        oled_ShowString(80, 4, "%", 16);
        oled_ShowString(80, 6, "lux", 16);
    }
}
...
void AutoContrl(void)
{
    if (dht11Data.temp_int >= Threshold.TempMax) {
		 ...
		 if (100-SensorData.SoliVal <= Threshold.SoliMin) {
       RELAY1_ON; 
    } else {
       RELAY1_OFF; 
    }
...
void ThresholdSet(void)
{
    ...
    if (IndexFlag1 <= 3) { //第一页
        ...
        oled_ShowNum(90, 4, Threshold.SoliMin, 2, 16);
        oled_ShowNum(90, 6, Threshold.LightMin, 2, 16);
        //单位显示

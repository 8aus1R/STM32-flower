...
void Manual_Contrl(void)
{
    ...
    if (IndexFlag <= 3) { //第一页
        ...
        if (System.Switch1) {
            oled_ShowString(72, 2, (u8 *)"ON ", 16);
        } else {
            oled_ShowString(72, 2, (u8 *)"OFF", 16);
        }

        if (System.Switch2) {
            oled_ShowString(72, 4, (u8 *)"ON ", 16);
        } else {
            oled_ShowString(72, 4, (u8 *)"OFF", 16);
        }

        if (System.Switch3) {
            oled_ShowString(72, 6, (u8 *)"ON ", 16);
        } else {
            oled_ShowString(72, 6, (u8 *)"OFF", 16);
        }
    } 

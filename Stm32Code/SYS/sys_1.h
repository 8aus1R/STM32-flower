...
typedef struct
{
    uint8_t LightVal;//光照强度
    uint8_t SoliVal;//土壤湿度
    uint16_t DustVal;//
		 u32 sgp30_dat;
...
typedef struct
{
    uint8_t LightMin;//光照最小值
    uint8_t TempMax;//温度最大值
    uint8_t HumiMin;//湿度最小值    
    uint8_t SoliMin;//土壤最小值
    uint16_t SGP30Max;//二氧化碳最大值
}THRESHOLD;

//系统标志位结构体
typedef struct
{
    u8 mqttflag;//MQTT消息发布标志

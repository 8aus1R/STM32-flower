...
void Sensor_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(AtmosClock,ENABLE);
	
	//时钟线推挽输出
	gpio.GPIO_Pin = AtmosCLK;	
	gpio.GPIO_Mode = GPIO_Mode_Out_PP;
	gpio.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(AtmosGPIO,&gpio);
...
//	GPIO_ResetBits(AtmosGPIO,DATA);
	GPIO_SetBits(AtmosGPIO,DATA);
	...
	//时钟线拉低 空闲时时钟线保持低电位
	GPIO_ResetBits(AtmosGPIO,AtmosCLK);
	
	delay_us(2);	
	
	//等待AD转换结束
	while(GPIO_ReadInputDataBit(AtmosGPIO,DATA));
	
	for(i=0;i<24;i++)
	{
		//时钟线拉高 开始发送时钟脉冲
		GPIO_SetBits(AtmosGPIO,AtmosCLK);
		...
		//读取一位数据
		if(GPIO_ReadInputDataBit(AtmosGPIO,DATA))
			value ++;
	}
...
{ 
	...
	SensorData.Presser=Weight_Real;

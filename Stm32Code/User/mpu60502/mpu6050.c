...
uint8_t MPU6050ReadID(void)
{
	...
	if(Re != 0x68)
	{
		u1_printf("MPU6050 dectected error!\r\n检测不到MPU6050模块，请检查模块与开发板的接线");
		return 0;
	}
	else
	{
		u1_printf("MPU6050 ID = %d\r\n",Re);
		oled_ShowNum(0,0,Re,8,16);
		return 1;
	}
...
//      sprintf ( cStr, "%8d%8d%8d",Accel[0],Accel[1],Accel[2] );	
//			oled_ShowString(0,0,cStr,16);
//      sprintf ( cStr, "%8d%8d%8d",Gyro[0],Gyro[1],Gyro[2] );	
//				oled_ShowString(0,2,cStr,16);
//      sprintf ( cStr, "Temp:%8.2f",Temp );	
//       	oled_ShowString(0,4,cStr,16);
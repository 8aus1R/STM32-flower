...
unsigned char DATA2[16]= {0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x07,0x80,0x69,0xff,0xff,0xff,0xff,0xff,0xff};
unsigned char status;
// 0x08 就是2扇区0区块（即第9块）
unsigned char addr=0x08;
// unsigned char addr=0x0F;
char sdhadhad[256];
...
******************/
 u8 Check_Card(void)
 {
		 ...
		 if(status == MI_OK)//选卡成功
    {
        ...
        {
             u1_printf("PcdAuthState(A) success\r\n");
        }
        else
        {
             u1_printf("PcdAuthState(A) failed\r\n");
						return 0;
        }
		// 0x61验证B密钥 块地址 密码 SN  块地址0x0B即2扇区3区块，可以替换成变量addr
				status = PcdAuthState(0x61, 0x0B, KEY_B, SN);
        if(status == MI_OK)//验证成功
        {
             u1_printf("PcdAuthState(B) success\r\n");
					return 1;
        }
        else
        {
             u1_printf("PcdAuthState(B) failed\r\n");
						return 0;
        }
    }
//		    if(status == MI_OK)//验证成功
...
返回值：无
******************/
 void Activation_CardInformation(void)
 {
		u8 ret;
	 CARD card;
	 ret=Check_Card();//验证卡
	 if(ret==1)
	 {
         ...
         if(card.stat==66)
         {
             u1_printf("此卡已激活,不需要重复激活.\r\n");
         }
         //卡已经挂失
         else if(card.stat==88)
         {
             u1_printf("此卡已激活,并挂失,锁定,请先解锁...\r\n");
         }
				 else 
				 {
						 card.stat=66;
						 ...
						 card.money=0;
             card.move=11;
					 strncpy((char*)card.name,"lihua",sizeof(card.name)-1);
					   
					  status = PcdWrite(addr, (u8*)&card);
						if(status == MI_OK)//写卡成功
						{
							 oled_ShowString(16*3,2*2,"OK",16);
							
							 u1_printf("PcdWrite() success\r\n");
						}
						else
						{
								 u1_printf("PcdWrite() failed\r\n");
						}
						
						PcdReset();
				 }
		 }
		 else  u1_printf("PcdRead() failed\r\n");
	 }
 ...
 void Remove_Card(void)
 {
	 ...
	 if(ret==1)
	 {
		 ...
		 if(status==MI_OK)
		 {
			 ...
			 card.money=0;
			 card.move=0;
			 strncpy((char*)card.name,"",sizeof(card.name)-1);
	 	 ...
	 	 if(ret==1)
	 {
		 ...
		 if(status==MI_OK)
		 {
			 if(card.stat==66)//检查卡是否激活
			 {
				...
				oled_ShowString(16*2,2*2,card.name,16);
							 u1_printf("名字%s\r\n",card.name);
				  oled_ShowCHinese(16*0,2*3,22);
				 oled_ShowCHinese(16*1,2*3,23);
				oled_ShowNum(16*2,2*3,card.money,6,16);
			         u1_printf("余额:%d\r\n",card.money);
			
				sprintf(sdhadhad,"{\"move\":%d,\"name\":%s,\"money\":%d,\"stat\":%d}",card.move,(char*)card.name,card.money,card.stat);  //构建回复湿度温度数据
	      mqtt_PublishQs0(P_TOPIC_NAME,sdhadhad,strlen(sdhadhad));	 
 
			 }
			  else if(card.stat==88)  u1_printf("此卡已挂失.请先解锁\r\n");

			 else  u1_printf("此卡没有激活\r\n");
			  PcdReset();
 }
		 }
	 }
	 	 ...
	 	 if(ret==1)
	 {
		 ...
		 if(status==MI_OK)
		 {
			 if(card.stat==66)//检查卡是否激活
			 {
				 ...
				 if(card.move==11){
					  oled_ShowCHinese(16*0,2*1,20);
				 oled_ShowCHinese(16*1,2*1,21);
				oled_ShowString(16*2,2*1,card.name,16);
					  u1_printf("门已开\r\n");
					  oled_ShowCHinese(16*0,2*2,12);
				    oled_ShowCHinese(16*1,2*2,24);
					  oled_ShowCHinese(16*2,2*2,25);
				        JDQ_OFF; 
		     }
...
返回值：无
******************/
 void Recharge_CardInformation(uint16_t count)
 {
	 	 ...
	 	 if(ret==1)
	 {
		 ...
		 if(status==MI_OK)
		 {
			 if(card.stat==66)//检查卡是否激活
			 {
				  ...
				  oled_ShowCHinese(16*3,2*2,22);
				  oled_ShowCHinese(16*4,2*2,23);
				 oled_ShowNum(16*5,2*2,card.money,3,16);
				 if(key3)//按键触发充值
				 {
				   ...
				   oled_ShowCHinese(16*4,2*2,23);
						...
						{
					 oled_ShowCHinese(16*0,2*3,13);
				   oled_ShowCHinese(16*1,2*3,14);
							 oled_ShowCHinese(16*2,2*3,31);
							 oled_ShowCHinese(16*3,2*3,32);
								 u1_printf("充值失败\r\n");
						}
				 }

			 }
			 else if(card.stat==88)  u1_printf("此卡已挂失.请先解锁后再充值.\r\n");

			 else  u1_printf("此卡没有激活.请先激活后再充值.\r\n");

			 PcdReset();
		 }
		 else  u1_printf("read failed\r\n");
	 }
 }
...
返回值：无
******************/
 void Consumption_CardInformation(uint16_t count)
 {
			 u8 ret;
	 CARD card;
	 ret=Check_Card();//验证卡
		 ...
		 if(status==MI_OK)
		 {
			 	if(card.stat==66)//检查卡是否激活
			 {
				 ...
				 oled_ShowCHinese(16*1,2*2,16);
				  ...
				  oled_ShowCHinese(16*3,2*2,22);
				  oled_ShowCHinese(16*4,2*2,23);
				   oled_ShowNum(16*5,2*2,card.money,4,16);
				  u1_printf("消费前的余额:%d\r\n",card.money);
				 if(key4)//按键触发扣款
				 {
					 //
...
返回值：无
******************/
 void locking_CardInformation(void)
 {
	 u8 ret;
	 CARD card;
	 ret=Check_Card();//验证卡
	 if(ret==1)
	 {
		 ...
		 if(status==MI_OK)
		 {
            ...
            card.stat=88;
				 status = PcdWrite(addr, (u8*)&card);
				if(status == MI_OK)   u1_printf("此卡已成功挂失\r\n");
				else   u1_printf("挂失失败\r\n");
			 }
			 else if(card.stat==88)  u1_printf("此卡已挂失\r\n");
			 else  u1_printf("此卡没有激活.请先激活.\r\n");
		 }
		 PcdReset();
	 }
 }
 ...
 /******************  
 功能：对卡解锁--去除挂失状态。将卡状态设置为66
 参数：无
返回值：无
******************/
 void Unlock_CardInformation(void) 
 {
	 u8 ret;
	 CARD card;
	 ret=Check_Card();//验证卡
	 if(ret==1)
	 {
		 ...
		 if(status==MI_OK)
		 {
				if(card.stat==66)  u1_printf("此卡已解锁.\r\n");
				else if(card.stat==88)
				{
					 card.stat=66;
					 status = PcdWrite(addr, (u8*)&card);
					if(status == MI_OK)   u1_printf("此卡已成功解锁\r\n");
					else   u1_printf("解锁失败\r\n");
				}
				else  u1_printf("此卡没有激活.请先激活.\r\n");
		 }
...
//    {
//        status = MI_ERR;
//		 u1_printf("Write the card after 1 second. Do not move the card!!!\r\n");
//		delay_ms(1000);
...
//    {
//        status = MI_ERR;
//		 u1_printf("RC522_Handle() run finished after 1 second!\r\n");
//        delay_ms(1000);
//    }
}
...
void RC522_Handle1(void)
{
    ...
    if(status == MI_OK)//读卡成功
    {
        ...
        if(status == MI_OK)//写卡成功
        {
             u1_printf("PcdWrite(RFID1) success\r\n");
        }
        else
        {
             u1_printf("PcdWrite(RFID1) failed\r\n");
			delay_ms(3000);
        }
    }
...
P1:	
	if(status == MI_OK)//写卡成功
    {
        ...
        if(status == MI_OK)//验证成功
        {
             u1_printf("PcdAuthState(A1) success\r\n");
        }
        else
        {
             u1_printf("PcdAuthState(A1) failed\r\n");
        }
		// 验证B密钥 块地址 密码 SN 
		status = PcdAuthState(0x61, test_addr, KEY_B1, SN);
        if(status == MI_OK)//验证成功
        {
             u1_printf("PcdAuthState(B1) success\r\n");
        }
        else
        {
             u1_printf("PcdAuthState(B1) failed\r\n");
        }
    }
	...
	if(status == MI_OK)//读卡成功
    {
        ...
        if(status == MI_OK)//写卡成功
        {
             u1_printf("PcdWrite(RFID2) success\r\n");
        }
        else
        {
             u1_printf("PcdWrite(RFID2) failed\r\n");
			delay_ms(3000);
        }
    }
    ...
    {
        status = MI_ERR;
		 u1_printf("RC522_Handle1() run finished after 1 second!\r\n");
        delay_ms(1000);
    }
}
...
char PcdAuthState ( u8 ucAuth_mode, u8 ucAddr, u8 * pKey, u8 * pSnr )
{
    ...
    //  u1_printf("char PcdAuthState ( u8 ucAuth_mode, u8 ucAddr, u8 * pKey, u8 * pSnr )\r\n");
    //  u1_printf("before PcdComMF522() ucComMF522Buf:%s\r\n", ucComMF522Buf);

    // 验证密钥命令
    cStatus = PcdComMF522 ( PCD_AUTHENT, ucComMF522Buf, 12, ucComMF522Buf, & ulLen );

    //  u1_printf("after PcdComMF522() ucComMF522Buf:%s\r\n", ucComMF522Buf);

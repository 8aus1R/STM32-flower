...
#include "stm32f10x.h"
 ...
 typedef struct CARD_INFO
 {
     ...
     u8  move;            //        88表示此卡挂失,无法再进行消费
     u16 money;    //金额. 第一次激活卡,就将金额清
	   u8 name[6];
...
u8 Check_Card(void);//检查卡的信息
void Activation_CardInformation(void);//激活卡片
void Recharge_CardInformation(uint16_t count);//充值金额
void Consumption_CardInformation(uint16_t count);//消费金额
void locking_CardInformation(void);
void Unlock_CardInformation(void);
void Remove_Card(void);//抹除卡
